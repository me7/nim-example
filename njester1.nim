import jester, re, random, strformat, json, os, db_sqlite
...
routes:
  ...
  get "/post":
    resp fmt"""
  <form name="input" action="{uri("/post", absolute = false)}" method="post">
  ...
  before re"/before/.*":
    if request.pathInfo.startsWith(re"^/before/restricted"):
